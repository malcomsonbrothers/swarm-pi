...
export function codexServiceTierFlagFilePath(options?: CodexServiceTierFlagFileOptions): string {
	...
	return homeDir && nodePath ? nodePath.join(homeDir, ".pi", "agent", "codex-service-tier") : "";
}
...
export function codexServiceTierFromFlagFile(
	...
	if (!flagFilePath || !nodeFs) return undefined;
	let contents: string;
	try {
		contents = nodeFs.readFileSync(flagFilePath, "utf8");
	} catch {
		return undefined;
	}
